...
def find_harness() -> str | None:
    for harness in aws_cli(["bedrock-agentcore-control", "list-harnesses"]).get("harnesses", []):
...
def main() -> None:
    ...
    ready = wait_for(lambda identifier: aws_cli(["bedrock-agentcore-control", "get-harness", "--harness-id", identifier])["harness"], harness_id)
